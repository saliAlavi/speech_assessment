...
def get_linear_classifier(feature_backbone, trainable=False):
    inputs = tf.keras.layers.Input(shape=(257, 1201, 1))
...
def plot_progress(hist):
    ...
    plt.grid()
    plt.savefig('aug_regression.png')
...
model.save_weights("/fs/scratch/PAS2622/ssl_based/new_stream/regression.h5")
