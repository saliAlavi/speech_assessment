...
def mapping_func(x):
    ftr_mat = x['log_mel_spectogram_0']
    # ftr_mat_norm = norm_layer(ftr_mat)
    ftr_mat_norm = ftr_mat
    mos_labl = x['mos']
...
with strategy.scope():
    ...
    projection.load_weights('mos_projection.h5')
...
def plot_progress(hist):
    ...
    plt.grid()
    plt.savefig('mos_aug_regression.png')
...
model.save_weights("/fs/scratch/PAS2622/ssl_based/new_stream/mos_regression.h5")
